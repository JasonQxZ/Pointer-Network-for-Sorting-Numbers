...
class linearRegression(object):
	def __init__(self,dimX,dimY,lossFunction=lambda x,y: 
		tf.where(tf.greater(x, y), (x-y), (y-x))  ,regFunction=tf.nn.l2_loss,reg_lambda=0.1):
		...
		self.yHat = tf.nn.xw_plus_b(self.x, self.W, self.b, name="yHat")
...
try:
	...
	tf.app.flags.DEFINE_integer('display_step', 50,"display_step")
...
xTr,yTr,zTr,xTe,yTe,zTe,prototypes = read_usaa()
...
dist0 = cdist(yhat,protoself)
zPred = unseen[np.argmin(dist0,axis=1)]
print('acc00:',sum(zPred==zTe)/len(zTe))
